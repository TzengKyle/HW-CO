...
int cache_sim_t::set_way(int set, int way)
{
    return set * ways + way;
}

int cache_sim_t::empty_place(int set)
{
    for (size_t i = 0; i < ways; i++)
    {
        if (tags[set_way(set, i)] == 0){ // 找到第一個empty的位置
            return set_way(set, i); 
        } 
                // 回傳在tags的index
    }
    return -1; // 滿了
...
void cache_sim_t::adjust_order(int hit_place, int set_end_place, uint64_t hit_tag)
{
    for (int i = hit_place + 1; i <= set_end_place; i++)
    {
        tags[i - 1] = tags[i];
    }

    tags[set_end_place] = hit_tag;
}

//_____________________

uint64_t *
cache_sim_t::check_tag(uint64_t addr)
{
    size_t idx = (addr >> idx_shift) & (sets - 1); // idx應該就是set
    size_t tag = (addr >> idx_shift) | VALID;

    for (size_t i = 0; i < ways; i++)
    {
        if (tag == (tags[idx * ways + i] & ~DIRTY)) // tag == (tags[idx * ways + i] & ~DIRTY)就是arr[i] == address
        {

            if (empty_place(idx) == -1) // 滿了
            {
                adjust_order(set_way(idx, i), set_way(idx, ways - 1), tag);
                return &tags[idx * ways + (ways - 1)];
            }
            else // 還沒滿
            {
                adjust_order(set_way(idx, i), empty_place(idx) - 1, tag);
                return &tags[empty_place(idx) - 1];
            }
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
    ...
    size_t way = 0;
    if (empty_place(idx) == -1)
    { // 滿了
        way = 0;
    }
    else
    {
        way = empty_place(idx) - (idx * ways);
    }
