...
int cache_sim_t::empty_place(int idx)
{
    for (size_t i = 0; i < ways; i++)
    {
        if (tags[set_way(idx, i)] == 0){// 找到第一個empty的位置
          return set_way(idx, i);
        }     // 回傳that empty's index in tags
    }
    return -1; // 滿了
}

int cache_sim_t::lf_place(int set) {
    size_t min = INT32_MAX;
    int lf_place = 0;
    for (int i = set_way(set, 0); i < set_way(set, ways-1); i++) {
        if (counter[i] < min) {
            min = counter[i];
            lf_place = i;
        }
...
uint64_t cache_sim_t::victimize(uint64_t addr)
{
  ...
  if (empty_place(idx) == -1) {
      way = lf_place(idx) - idx * ways;
      counter[lf_place(idx)] = 0;
  }
  else {
