...
{
  size_t idx = (addr >> idx_shift) & (sets-1);
  size_t way = target[idx]; //idx represent the corresponded set and target[idx] means the block which would be victim
  target[idx] = (target[idx]+1) % ways; //ensure the way won't be bigger than ways (instinct)
